...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    (or one of the local-server aliases that resolve to ``custom`` — vllm, llamacpp, …; "ollama"
    is a first-class provider now, not one of these aliases — see hermes_cli/auth.py),
    ...
    # otherwise a legit LAN/WireGuard endpoint silently falls through to
    # OpenRouter.
    try:
...
def _resolve_named_custom_runtime(
    *,
    ...
    # fall through to OpenRouter. ("ollama" now resolves to its own
    # first-class provider — see hermes_cli/auth.py — and no longer takes
    # this path at all.)
...
def _resolve_openrouter_runtime(
    *,
    ...
    # llamacpp, …) follow the same base_url trust + routing rules as a bare
    # `provider: custom`. Normalising here keeps every check below —
    # `requested_norm == "custom"`, the trust check, the pool gate up the
    # stack — alias-aware without duplicating the alias map. ("ollama" is
    # a first-class provider now — see hermes_cli/auth.py — and resolves
    # to itself, not "custom".)
    if requested_norm and requested_norm != "custom":
        try:
            from hermes_cli.auth import resolve_provider as _resolve_provider

...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        if row.get("is_user_defined"):
            continue
        # Local providers (e.g. ollama) never go through a real auth flow —
        # "authenticated" here just means "the live probe found models right
        # now", not that the user configured anything. Always expose
        # auth_type/key_env for these so the onboarding / "I have an API
        # key" picker can surface them even while the probe is currently
        # succeeding (the common case once Ollama is actually running).
